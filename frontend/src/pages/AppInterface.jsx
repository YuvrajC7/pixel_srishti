...
export default function AppInterface() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', text: data.reply, metadata: data.metadata }]);
    } catch (error) {
      console.error(error);
      setMessages(prev => [...prev, { role: 'assistant', text: 'Error connecting to the orchestration backend.' }]);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="h-screen w-full bg-[#030712] text-slate-100 flex flex-col font-sans overflow-hidden selection:bg-blue-500/30">
      ...
      <div className="flex-1 relative flex overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
           <MapContainer center={mapCenter} zoom={4} className="h-full w-full" zoomControl={false}>
              <TileLayer
        ...
        {/* FLOATING CONFIGURATION WIDGET (Top Left) */}
        <div className="absolute top-6 left-6 z-10 w-[380px] bg-[#02040A]/80 backdrop-blur-xl border border-white/10 rounded-3xl p-5 shadow-2xl">
